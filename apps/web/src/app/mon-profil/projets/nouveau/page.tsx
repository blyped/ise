...
export const metadata = { title: frProfile.projectForm.addTitle };
...
/** ISE-026 — Ajouter un projet. */
export default async function NewProjectPage() {
  const context = await requireProfile();

  const data = context.ok
    ? await Promise.all([loadSectors(context.correlationId), loadCountries(context.correlationId)])
    : null;
  ...
  return (
    <ProfilePage
      ...
      title={frProfile.projectForm.addTitle}
    ...
    >
      {data === null ? null : !data[0].ok || !data[1].ok ? (
        ...
        <ProjectForm project={null} sectors={data[0].data} countries={data[1].data} />
      )}
    </ProfilePage>
  );
}
