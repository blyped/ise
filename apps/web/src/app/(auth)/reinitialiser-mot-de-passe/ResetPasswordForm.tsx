...
export function ResetPasswordForm() {
  const [state, formAction, isPending] = useActionState(resetPasswordAction, initialFormState);
  ...
  return (
    <form action={formAction} onSubmit={onSubmit} noValidate className="flex flex-col gap-5">
      ...
      <Button
        ...
        fullWidth
        loading={isPending}
        loadingLabel={fr.auth.resetPassword.submitPending}
      >
        {fr.auth.resetPassword.submit}
      </Button>
    </form>
  );
}
