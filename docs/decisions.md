...
| D-184 → D-212 | [`decisions/10-dons-organisations-cadrage-et-pastilles.md`](decisions/10-dons-organisations-cadrage-et-pastilles.md) |
...
- **§60. Cadrage vertical du portrait public — diagnostic et correctif, zoom réducteur** — D-205, l'ancienne formule `object-position` + `transform: scale()` ne laissait de marge de déplacement que sur l'axe où la photo débordait déjà du cadre
- **§61. Cadrage ajustable étendu à la photo de profil (avatar)** — D-206, même mécanisme que le portrait public, appliqué au cockpit (en-tête) et à l'écran d'édition
- **§62. Nettoyage, dédoublonnage et harmonisation du référentiel organisations, ajout d'Optimum Conseil** — D-207, fusion de 8 doublons supplémentaires (au-delà des 21 de D-194/0143), liste des cas douteux volontairement non tranchés, durcissement de la picklist CMS et de `set_landing_organization()`
...
- **§67. Bug — cadrans de recadrage (médaillon + rectangle) qui découpaient déjà la photo avant tout réglage** — D-212, le wrapper de cadrage dimensionnait toujours au rapport du CADRE, jamais à celui de la PHOTO ; correctif générique (`imageAspect`/`frameAspect`), nouvelles colonnes `avatar_width`/`avatar_height` (migration 0152, même forme que `public_photo_width`/`height` de 0120), corrigé dans l'éditeur ET dans la vignette « ISE du jour » de l'accueil
- **§68. Taille max de photo relevée à 5 Mo, bug d'affichage de l'avatar corrigé, cadrage optimisé** — D-213 (5 Mo), D-214 (avatar absent de `/mon-profil`), D-215 (zoom minimum = photo entière, zoom maximum calculé par photo et par cadre via `photoCropZoomMax`)
...
- **§73. Circulation intelligente des signaux** — D-221 : le moteur de matching (paliers immediate/digest/none) enfin branché à la publication — fan-out in-app ciblé (score ≥ 75, plafond 50), courrier hebdomadaire des signaux (cron Vercel + `weekly_digest_*`), jamais d'e-mail immédiat par signal
- **§74. Guichet unique « Signaler »** — D-222 : `/signaler`, 17 intentions en langage naturel routant vers les 6 surfaces existantes (~23 cas de collaboration couverts sans nouveau module) ; libellé « Appel d'offres / marché »
