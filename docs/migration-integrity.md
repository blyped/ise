...
## Annexe B — Clôture des 20 divergences (12 août 2026)

Suite du contrôle du 8 août 2026 ci-dessus. Objectif : pour chacune des 20 migrations
listées comme divergentes au niveau N3, identifier la nature exacte de l'écart, la
classifier, et trancher — **sans éditer aucun fichier de `supabase/migrations/`**
(règle rappelée en tête de ce document et dans `supabase/migrations/README.md`).

### B.1 Méthode

Pour chaque migration, le SQL réellement appliqué (`supabase_migrations.schema_migrations.statements`)
a été comparé au fichier du dépôt avec une **forme super-canonique**, plus stricte que celle
du §1 : en plus de retirer les commentaires pleine ligne et de normaliser les espaces, elle

1. retire aussi les commentaires `--` en fin de ligne de code (limite connue du §1) ;
2. retire intégralement chaque instruction `comment on ... is '...';` (les libellés de
   documentation n'ont aucun effet exécutable, quel que soit leur contenu) ;
3. normalise tout délimiteur de dollar-quoting (`$$`, `$fn$`, `$mig$`, `$p$`, …) vers un
   jeton commun, ces noms étant purement conventionnels pour PostgreSQL.

Si les empreintes coïncident après cette normalisation, l'écart est **prouvé** limité aux
trois catégories ci-dessus — aucune lecture manuelle de diff n'est nécessaire pour l'affirmer.
Si elles ne coïncident pas, l'écart touche du SQL exécutable et doit être examiné à la main.

### B.2 Verdict par migration

**15 des 20 migrations ont été vérifiées intégralement** (fichier du dépôt récupéré depuis
`main`, SQL appliqué récupéré depuis `supabase_migrations.schema_migrations`, comparaison
super-canonique rejouée) :

| Migration | Empreinte super-canon (dépôt = base) | Nature de l'écart | Classification |
| --- | --- | --- | --- |
| `0030_search_engine` | `aa12dc28...` égales | `comment on` plus verbeux dans le dépôt (ex. « SECURITY DEFINER : … n'a aucune politique**, la table est fermée aux membres**. » vs appliqué, plus court) sur 5 fonctions ; délimiteurs `$$` (dépôt) vs `$fn$`/`$mig$` (appliqué) | Cosmétique + forme équivalente |
| `0031_matching_engine` | `f8c7f6bc...` égales | Commentaire de fin de ligne `-- D-43` ajouté dans le dépôt ; `comment on function match_profiles` réécrit plus long dans le dépôt (« seuls relevance_label et reasons sortent… ») | Cosmétique |
| `0032_profile_completion` | (sondage détaillé §0 ci-dessus) | `comment on column` : « Recalibrable par le back-office » (dépôt) vs « Recalibrable sans migration (D-71) » (appliqué) — texte différent, pas juste reformaté | Cosmétique (libellé) — **voir note ci-dessous** |
| `0033_search_matching_performance` | `2d7e004d...` égales | Même profil que 0031 : `comment on` plus détaillé dans le dépôt, forme littérale concaténée | Cosmétique |
| `0034_matching_set_based` | `47f22f1a...` égales | Identique à 0031/0033 (même paire de fonctions, même commentaire `-- D-43` et même `comment on` rallongé) | Cosmétique |
| `0036_member_profile_and_saved_searches` | `4c5eb475...` égales | `comment on` reformaté (littéraux concaténés) ; l'en-tête du fichier appliqué porte encore l'ancien nom `0035_member_profile_and_saved_searches` (renumérotation §2.3), sans effet exécutable | Cosmétique |
| `0038_get_member_profile_field_list_fix` | `c2ab9b9e...` égales | `comment on function get_member_profile` reformaté en littéraux concaténés | Cosmétique |
| `0040_rls_network_calls` | `87a3af70...` égales | `comment on function/column` reformatés, délimiteurs `$$` | Cosmétique |
| `0042_rls_internships` | `89ed0cd6...` égales | `comment on function can_see_internship_need` reformaté | Cosmétique |
| `0043_rls_mentorship` | (sondage détaillé §0 ci-dessus) | `execute format('grant update(dismissed_at) …', …)` (dépôt) vs `execute 'grant update(dismissed_at) … to authenticated'` (appliqué) — même effet SQL, forme paramétrée vs littérale | Forme équivalente |
| `0045_rls_projects_consortiums` | `aa4b1c00...` égales | `comment on function can_see_project` reformaté | Cosmétique |
| `0046_rls_news_events` | `8531f06a...` égales | `comment on function/column` (dont `events.online_url_private`, `get_event_online_url`) reformatés | Cosmétique |
...
**textes différents** (D-71 ajouté dans un cas, un mot ajouté dans l'autre). Cela reste sans
effet exécutable — `comment on ... is` n'influence aucune requête, aucune RLS, aucune
fonction — mais un futur lecteur de `pg_description` en base verra un texte différent de
celui du fichier. C'est exactement le type d'écart que le principe du §B.4 corrige.

### B.4 Principe retenu pour trancher « quelle version fait foi »

**La base de production fait foi pour le comportement réel** : c'est le SQL qui y est stocké
qui s'exécute à chaque appel, indépendamment de ce que porte le fichier. Mais **le dépôt doit
être corrigé pour refléter fidèlement l'appliqué**, jamais l'inverse : réappliquer aujourd'hui
un fichier déjà exécuté ne changerait rien à la base (les objets existent déjà, `create or
replace` recréerait un résultat strictement identique), alors que laisser le dépôt diverger
prive le projet de sa seule preuve fiable de ce qui tourne réellement. C'est le principe déjà
appliqué en §4 (`0024_seed_taxonomy_business`) et en §2.3 (`0035`/`0036`).

### B.5 Comment corriger le dépôt sans éditer un fichier déjà appliqué
